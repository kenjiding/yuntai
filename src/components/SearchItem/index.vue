<template>
  <div :class="$style['headers-search']">
    ...
    <searchPage
      ...
      @search="searchPageHandle"
      :history-name="historyName"
      @clearSearch="clickHistoryValueAndClearKey"
      @historyValue="clickHistoryValueAndClearKey"/>
  </div>
</template>
...
import searchPage from './components/HistoryPage';
...
export default {
  ...
  methods: {
    clickHistoryValueAndClearKey (val) {
      this.showVisible = false;
      this.value = val;
      this.$emit('search', val);
    },
    searchPageHandle (val) {
      this.value = val;
